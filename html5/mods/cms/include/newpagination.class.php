...
class Newpagination {
	...
	function create_links()
	{
		...
		$output='
		<form action="'.$this->base_url.'" class="page_nav_jump"  method="get" id="pageForm" onsubmit="document.getElementById(\'gotopagebutton\').click(); return false;">
			<dl>
            	<dt>
                    <dd><a href="'.$this->base_url.(($this->cur_page  > 1) ? $this->cur_page - 1 : 1).'"><<</a></dd>
                    <dd>第 '.$this->cur_page.' 页</dd>
                    <dd><a href="'.$this->base_url.(($this->cur_page < $num_pages) ? $this->cur_page + 1 : $num_pages).'">>></a></dd>
                    <dd class="page_tzw">跳到 <input type="text" class="txt1" data-theme="f" id="goto_page" name="goto_page"> 页</dd>
                    <dd><input id="gotopagebutton" onClick="var page = parseInt(document.getElementById(\'goto_page\').value); if(page<1 || isNaN(page)){page=1;}else if(page>'.$num_pages.'){page='.$num_pages.';}  location.href=\''.$this->base_url.'\'+page;" type="button" class="submit" value="跳转"></dd>
                    <dd>第'.$this->cur_page.'页/共'.ceil($this->total_rows/$this->per_page).'页</dd>
                </dt>
            </dl>
		</form>
			';
