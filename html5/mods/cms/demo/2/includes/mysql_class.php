...
class Db_class {
	...
	function connect($dbhost, $dbuser, $dbpw, $dbname, $pconnect = 0) {
        ...
        if($this->server_info() > '4.1' && DB_ENCODING)
			mysql_query("SET NAMES '" . DB_ENCODING . "'", $this->link);
     ...
     * @return resource
     */
     function insert_new($table, $inlist ,$where=null) {
        if(!$table) return;
	...
	function halt($msg = '') {
        //global $charset;
        if(defined('SET_ERROR_REPORT') and SET_ERROR_REPORT==1){
        }else{
			exit();
        }
		$message = "<html>\n<head>\n";
